...
class CastomCell21: UITableViewCell{
    
    @IBOutlet weak var nazwa2: UILabel!
    @IBOutlet weak var opis2: UILabel!
    @IBOutlet weak var cena2: UILabel!
    @IBOutlet weak var uwaga2: UIImageView!
    @IBOutlet weak var ulubione2: UIButton!
    
}

class CastomCell21a: UITableViewCell{
...
class ChildViewController21: UIViewController, IndicatorInfoProvider{
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refControl.addTarget(self, action: #selector(ChildViewController21.aktualizacjaDanych(_:)), for: .valueChanged)
...
//        AppDelegate.which_page = 1 //zapamiętanie numeru strony - żeby wrócić np. po zmianie języka
    ...
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo (title: "L_PRZYSTAWKI".localized()) //NSLocalizedString("L_SNIADANIA", comment: " ŚNIADANIA")
    }
...
//====================================
extension ChildViewController21: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if listaDan2[indexPath.row].foto != "co.jpg"{
            ...
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "CastomCell21a", for: indexPath) as? CastomCell21a else {
                fatalError("Komórka nie jest instancją CastomCell21")
            }
            
            //żeby nie znikało tło pod napisami po wybraniu celi
            cell.selectionStyle = UITableViewCellSelectionStyle.none
            
            cell.nazwa2.text = listaDan2[indexPath.row].nazwa
            cell.opis2.text = listaDan2[indexPath.row].opis
            //cell.cena2.text = listaDan2[indexPath.row].cena
            ...
            if listaDan2[indexPath.row].cena != "wr"{
                ...
                if Shared.shared.uz_id! == "" {
                    cell.cena2.text = "\(numCena! ) " + "L_PLN".localized()//cena menu + opakowania
                }else{
                    cell.cena2.text = numCena!
                }
            }else{
                cell.cena2.text = listaDan2[indexPath.row].cena
            }
            ...
            if listaDan2[indexPath.row].foto == "co.jpg" {
                cell.foto2.image = UIImage(named: "brak2.jpg")
            }else{
                //pobranie zdjęcia dania
                nh = Network()
                nh?.getPictureFromServer(imageName: listaDan2[indexPath.row].foto, callback: { (image:UIImage)->Void in
                    DispatchQueue.main.async{
                        cell.foto2.image = image
                    }
                })
            }
            ...
            //pobranie komórki z tabeli
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "CastomCell21", for: indexPath) as? CastomCell21 else {
                fatalError("Komórka nie jest instancją CastomCell21")
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //print(listaDan2[indexPath.row].foto)
        //print(listaDan2[indexPath.row].alergeny)
        // print(listaDan2[indexPath.row].nazwa)
        ...
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            performSegue(withIdentifier: "showDetails21", sender: self) //przejście do TabBarController
