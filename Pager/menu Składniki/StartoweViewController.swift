...
class StartoweCell: UITableViewCell{
    ...
    @IBOutlet weak var s01Button: UIButton!
...
class StartoweViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {

    
    @IBOutlet weak var tableViewStartowe: UITableView!
   ...
   func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "StartoweCell", for: indexPath) as? StartoweCell else {
                fatalError("Komórka nie jest instancją StartoweCell")
            }
                ...
                //buźki
                cell.s01Button.tag = indexPath.row
                if Shared.shared.formy[indexPath.row].ud_lubi == 1{
                    cell.s01Button.setImage(UIImage(named: "k_01"), for: .normal)
                }else{ cell.s01Button.setImage(UIImage(named: "sz_01"), for: .normal)}
                cell.s01Button.addTarget(self, action: #selector(self.buzia01), for: .touchUpInside)
...
//                cell.b10Button.addTarget(self, action: #selector(self.buzia10), for: .touchUpInside)
  ...
  func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
    ...
    @IBAction func buzia01(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "1", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
            ...
            self.downloadJSON_rodzaje{
                ...
                //zapisanie kategorii składników do bazy lokalnej 'rodzaj'
    ...
    @IBAction func buzia02(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "2", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia03(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "3", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia04(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "4", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia05(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "5", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia06(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "6", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia07(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "7", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia08(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "8", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia09(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "9", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    @IBAction func buzia10(_ sender: UIButton) {
        ...
        //update ocenionego składnika startowego na serwerze cobytu.com
        let res = self.aktualizujSkladnikStartowy(us_lubi: "10", us_ss_id: String(Shared.shared.formy[sender.tag].fd_id))
        if res == 1 { //jeżeli udało się zapisać zmiany na serwerze
            //odświeżenie celi z ocenioną nazwą
            self.tableViewStartowe.reloadRows(at: [[0, sender.tag]], with: .none)
    ...
    //----------------------------------------------------------
    //aktualizacja ocenianego składnika startowego na serwerze
    //----------------------------------------------------------
    func aktualizujSkladnikStartowy(us_lubi: String, us_ss_id: String)  -> Int  {
        ...
        let postString = ["us_ss_id": us_ss_id,
                          "us_lubi": us_lubi,
                          "us_uz_id": Shared.shared.uz_id,
                          "us_ss_sg": String(Shared.shared.rd_id)] as [String: String]
        do{
