...
class ChildViewController5: UIViewController, IndicatorInfoProvider{
    ...
    //let brak_foto = UIImage(named: "brak.jpg")
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refControl.addTarget(self, action: #selector(ChildViewController5.aktualizacjaDanych(_:)), for: .valueChanged)
    ...
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "L_DANIA_GLOWNE".localized())
    }
...
//====================================
extension ChildViewController5: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "Meal5TableViewCell", for: indexPath) as? Meal5TableViewCell else {
            fatalError("Komórka nie jest instancją Meal5TableViewCell")
        }
        ...
        cell.starsView5.settings.updateOnTouch = false
        cell.starsView5.rating = Double(listaDan5[indexPath.row].srednia)!
        cell.starsView5.text = listaDan5[indexPath.row].srednia
        //załadowanie danych dań
        cell.nameLabel5.text = listaDan5[indexPath.row].nazwa
        //cell.cenaLabel5.text = listaDan5[indexPath.row].cena
        ...
        if listaDan5[indexPath.row].cena != "wr"{
            ...
            cell.cenaLabel5.text = "\(numCena! ) " + "L_PLN".localized()//cena menu + opakowania
        }else{
            cell.cenaLabel5.text = listaDan5[indexPath.row].cena
        }
        
        cell.czasLabel5.text = "\(listaDan5[indexPath.row].czas) " + "L_MIN".localized()
        cell.wagaLabel5.text = "\(listaDan5[indexPath.row].waga) " + "L_G".localized()
        cell.kcalLabel5.text = "\(listaDan5[indexPath.row].kcal) " + "L_KCAL".localized()
        //print (listaDan5[indexPath.row].foto)
        ...
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            if listaDan5[indexPath.row].foto == "co.jpg" {
                cell.photoImageView5.image = UIImage(named: "brak2.jpg")
            }else{
                let bez_m = listaDan5[indexPath.row].foto.replacingOccurrences(of: "_m.", with: ".") //nazwa foto bez _m.
                //pobranie zdjęcia dania
                nh = Network()
                nh?.getPictureFromServer(imageName: bez_m, callback: { (image:UIImage)->Void in
                    DispatchQueue.main.async{
                        cell.photoImageView5.image = image
                    }
                })
            }
        }else{ //jeżeli nie ma Internetu
            cell.photoImageView5.image = UIImage(named: "brak2.jpg")//wczytanie zdjęcia tymczasowego
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.reachability = Reachability.init()
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            performSegue(withIdentifier: "showDetails5", sender: self) //przejście do TabBarController
        }else{ //jeżeli nie ma Internetu
            displayAlert()
        }
    }
