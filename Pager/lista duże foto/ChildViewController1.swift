...
//====================================
extension ChildViewController1: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "Meal1TableViewCell", for: indexPath) as? Meal1TableViewCell else {
            fatalError("Komórka nie jest instancją Meal1TableViewCell")
        }
        ...
        cell.starsView.settings.updateOnTouch = false
        cell.starsView.rating = Double(listaDan1[indexPath.row].srednia)!
        cell.starsView.text = listaDan1[indexPath.row].srednia
        //załadowanie danych dań
        cell.nameLabel1.text = listaDan1[indexPath.row].nazwa
        //cell.cenaLabel1.text = listaDan1[indexPath.row].cena
        ...
        if listaDan1[indexPath.row].cena != "wr"{
            ...
            cell.cenaLabel1.text = "\(numCena! ) " + "L_PLN".localized()//cena menu + opakowania
        }else{
            cell.cenaLabel1.text = listaDan1[indexPath.row].cena
        }
        cell.czasLabel1.text = "\(listaDan1[indexPath.row].czas) " + "L_MIN".localized()
        cell.wagaLabel1.text = "\(listaDan1[indexPath.row].waga) " + "L_G".localized()
        cell.kcalLabel1.text = "\(listaDan1[indexPath.row].kcal) " + "L_KCAL".localized()
        ...
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            if listaDan1[indexPath.row].foto == "co.jpg" {
                cell.photoImageView1.image = UIImage(named: "brak2.jpg")
            }else{
                 let bez_m = listaDan1[indexPath.row].foto.replacingOccurrences(of: "_m.", with: ".") //nazwa foto bez _m.
                //pobranie zdjęcia dania
                nh = Network()
                nh?.getPictureFromServer(imageName: bez_m, callback: { (image:UIImage)->Void in
                    DispatchQueue.main.async{
                        cell.photoImageView1.image = image
                    }
                })
            }
        }else{ //jeżeli nie ma Internetu
            cell.photoImageView1.image = UIImage(named: "brak2.jpg")//wczytanie zdjęcia tymczasowego
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if selectedIdDania != "0" {
            ...
            self.reachability = Reachability.init()
            if((self.reachability!.connection) != .none){ //jeżeli jest Internet
                performSegue(withIdentifier: "showDetails1", sender: self) //przejście do TabBarController
            }else{ //jeżeli nie ma Internetu
                displayAlert()
            }
        }
