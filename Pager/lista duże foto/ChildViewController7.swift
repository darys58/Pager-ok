...
class ChildViewController7: UIViewController, IndicatorInfoProvider{
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refControl.addTarget(self, action: #selector(ChildViewController7.aktualizacjaDanych(_:)), for: .valueChanged)
...
//====================================
extension ChildViewController7: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "Meal7TableViewCell", for: indexPath) as? Meal7TableViewCell else {
            fatalError("Komórka nie jest instancją Meal7TableViewCell")
        }
        ...
        cell.starsView7.settings.updateOnTouch = false
        cell.starsView7.rating = Double(listaDan7[indexPath.row].srednia)!
        cell.starsView7.text = listaDan7[indexPath.row].srednia
        //załadowanie danych dań
        cell.nameLabel7.text = listaDan7[indexPath.row].nazwa
        //cell.cenaLabel7.text = listaDan7[indexPath.row].cena
        ...
        if listaDan7[indexPath.row].cena != "wr"{
            ...
            cell.cenaLabel7.text = "\(numCena! ) " + "L_PLN".localized()//cena menu + opakowania
        }else{
            cell.cenaLabel7.text = listaDan7[indexPath.row].cena
        }
        
        cell.czasLabel7.text = "\(listaDan7[indexPath.row].czas) " + "L_MIN".localized()
        cell.wagaLabel7.text = "\(listaDan7[indexPath.row].waga) " + "L_G".localized()
        cell.kcalLabel7.text = "\(listaDan7[indexPath.row].kcal) " + "L_KCAL".localized()
        ...
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            if listaDan7[indexPath.row].foto == "co.jpg" {
                cell.photoImageView7.image = UIImage(named: "brak2.jpg")
            }else{
                 let bez_m = listaDan7[indexPath.row].foto.replacingOccurrences(of: "_m.", with: ".") //nazwa foto bez _m.
                //pobranie zdjęcia dania
                nh = Network()
                nh?.getPictureFromServer(imageName: bez_m, callback: { (image:UIImage)->Void in
                    DispatchQueue.main.async{
                        cell.photoImageView7.image = image
                    }
                })
            }
        }else{ //jeżeli nie ma Internetu
            cell.photoImageView7.image = UIImage(named: "brak2.jpg")//wczytanie zdjęcia tymczasowego
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.reachability = Reachability.init()
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            performSegue(withIdentifier: "showDetails7", sender: self) //przejście do TabBarController
        }else{ //jeżeli nie ma Internetu
            displayAlert()
        }
    }
