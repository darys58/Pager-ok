...
class ChildViewController4: UIViewController, IndicatorInfoProvider{
    ...
    lazy var refreshControl: UIRefreshControl = {
        ...
        refControl.addTarget(self, action: #selector(ChildViewController4.aktualizacjaDanych(_:)), for: .valueChanged)
    ...
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "L_SALATKI".localized())
    }
...
//====================================
extension ChildViewController4: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "Meal4TableViewCell", for: indexPath) as? Meal4TableViewCell else {
            fatalError("Komórka nie jest instancją Meal4TableViewCell")
        }
        ...
        cell.starsView4.settings.updateOnTouch = false
        cell.starsView4.rating = Double(listaDan4[indexPath.row].srednia)!
        cell.starsView4.text = listaDan4[indexPath.row].srednia
        //załadowanie danych dań
        cell.nameLabel4.text = listaDan4[indexPath.row].nazwa
        //cell.cenaLabel4.text = listaDan4[indexPath.row].cena
        ...
        if listaDan4[indexPath.row].cena != "wr"{
            ...
            cell.cenaLabel4.text = "\(numCena! ) " + "L_PLN".localized()//cena menu + opakowania
        }else{
            cell.cenaLabel4.text = listaDan4[indexPath.row].cena
        }
        cell.czasLabel4.text = "\(listaDan4[indexPath.row].czas) " + "L_MIN".localized()
        cell.wagaLabel4.text = "\(listaDan4[indexPath.row].waga) " + "L_G".localized()
        cell.kcalLabel4.text = "\(listaDan4[indexPath.row].kcal) " + "L_KCAL".localized()
        ...
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            if listaDan4[indexPath.row].foto == "co.jpg" {
                cell.photoImageView4.image = UIImage(named: "brak2.jpg")
            }else{
                 let bez_m = listaDan4[indexPath.row].foto.replacingOccurrences(of: "_m.", with: ".") //nazwa foto bez _m.
                //pobranie zdjęcia dania
                nh = Network()
                nh?.getPictureFromServer(imageName: bez_m, callback: { (image:UIImage)->Void in
                    DispatchQueue.main.async{
                        cell.photoImageView4.image = image
                    }
                })
            }
        }else{ //jeżeli nie ma Internetu
            cell.photoImageView4.image = UIImage(named: "brak2.jpg")//wczytanie zdjęcia tymczasowego
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.reachability = Reachability.init()
        if((self.reachability!.connection) != .none){ //jeżeli jest Internet
            performSegue(withIdentifier: "showDetails4", sender: self) //przejście do TabBarController
        }else{ //jeżeli nie ma Internetu
            displayAlert()
        }
    }
